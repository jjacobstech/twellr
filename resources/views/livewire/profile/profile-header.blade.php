...
new class extends Component {
    ...
    {
        $data = (object) $this->validate(['comment' => ['required', 'string']]);
        $comment = Comment::create([
            'user_id' => Auth::id(),
            'product_id' => $this->design,
        ...
        if (!$comment) {
            ...
            $this->comment = '';
            $this->comments = Comment::where('product_id', $this->design)->with('user')->get();
        }
    }
...
copyReferralLink(){
    ...
    <div wire:show="showComments"
        class="fixed z-50 inset-0 flex items-center justify-center bg-gradient-to-br from-navy-blue/80 via-black/70 to-golden/80 backdrop-blur-lg transition-all duration-500">
        <section wire:click.away="showComments = false"
            class="relative rounded-2xl bg-white border-0 shadow-2xl p-6 my-6 mx-auto w-[90%] max-w-2xl animate-fade-in">
            <!-- Close Button -->
            <button wire:click="showComments = false"
                class="absolute top-4 right-4 text-gray-400 hover:text-navy-blue transition-colors duration-200 text-2xl focus:outline-none">
                &times;
            </button>
            <div class="flex items-center gap-2 mb-2">
                <svg class="w-7 h-7 text-golden" fill="none" stroke="currentColor" stroke-width="2"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round"
                        d="M17 8h2a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h2"></path>
                    <rect width="8" height="8" x="8" y="2" rx="2"></rect>
                </svg>
                <h3 class="font-os text-2xl font-extrabold text-navy-blue tracking-wide">Comments</h3>
            </div>

            <div class="max-h-60 overflow-y-scroll scrollbar-thin scrollbar-thumb-navy-blue scrollbar-track-gray-300  pr-2 custom-scrollbar space-y-4">
                @forelse ($comments as $comment)
                    <!-- Comment -->
                    <div
                        class="flex items-start gap-3 bg-gray-50 rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow duration-200">
                        @if (!empty($comment->user->avatar) && str_contains($comment->user->avatar, 'https://'))
                            <x-bladewind.avatar class="ring-2 ring-golden border-0 aspect-square" size="large"
                                :image="$comment->user->avatar" />
                        @else
                            @if ($comment->user->avatar)
                                <x-bladewind.avatar class="ring-2 ring-golden border-0" size="large"
                        ...
                        <div class="flex-1">
                            <div class="flex items-center gap-2">
                                <span class="font-semibold text-navy-blue text-base">
                                    {{ "{$comment->user->firstname} {$comment->user->lastname}" }}
                                </span>
                                <span class="text-xs text-golden font-semibold">
                ...
                @empty
                    ...
                    <div class="flex flex-col items-center justify-center mt-8">
                        ...
                        <p class="text-lg font-bold text-gray-400">No Comments Yet.</p>
                    </div>
                @endforelse
            </div>
            ...
            <!-- Comment Form -->
            <form
                class="mt-6 bg-gradient-to-r from-navy-blue/10 to-golden/10 rounded-xl p-4 shadow-inner">
                <div class="mb-3">
                    <label for="comment" class="block text-navy-blue font-semibold mb-1">Add a Comment</label>
                    <textarea id="comment" wire:model="comment"
                        class="bg-white shadow p-3 w-full rounded-lg border border-golden focus:ring-2 focus:ring-golden focus:outline-none text-gray-700 resize-none transition"
                        rows="3" maxlength="300" placeholder="Write something nice..." required></textarea>
                </div>
                <div class="flex justify-between gap-5">
                     <div class="px-5 py-3 relative w-[60%]">
                        <x-input-error :messages="$errors->get('comment')" class="validator absolute" />
                     </div>
                    <x-mary-button spinner wire:click="sendComment"
                                class="w-[40%] px-4 py-3 font-medium uppercase transition duration-200 border-none text-white rounded bg-navy-blue hover:bg-golden hover:text-white">
                                Post Comment
                            </x-mary-button>
                </div>
            </form>
