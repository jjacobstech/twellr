...
$bayesianAverage = (($count / ($count + $minRatings)) * $average) +
                   (($minRatings / ($count + $minRatings)) * $globalAverage);
...
$filter = function ($filterValue = '') {
    ...
    return $this->filterTerm = $filterValue;
};
...
<div class="w-screen h-screen scrollbar-thin scrollbar-track-white scrollbar-thumb-navy-blue">
    ...
    <!-- Marketplace -->
    <div wire:show="marketplace" class="grid w-screen h-full space-x-1 lg:flex">

        <x-market-place-sidebar :locations="$locations" :categories="$categories" :filterTerm="$filterTerm" />
        <div
            class="relative bg-white w-screen pb-8 md:pb-0 md:w-[72%] lg:w-[80%] md:h-screen  lg:py-4 overflow-y-scroll scrollbar-none">
            <div x-cloak="display:hidden"
                class="relative grid justify-center w-full grid-cols-3 gap-5 px-5 mb-16 md:h-screen lg:hidden md:grid-cols-2">
                        ...
                        <div
                            ...
                            @else
                                <p class="text-black scrollbar-none">Follow
                                    {{ $order->designer->firstname . ' ' . $order->designer->lastname }} to be able to
                                    Comment </p>
                            @endif
                        </div>
                    @endif
                                    ...
                                    <div class="w-full">
                                        ...
                                        <select id="size" required wire:model.live="size"
                                            ...
                                            @foreach (['xs', 's', 'm', 'l', 'xl'] as $size)
                                                <option class="uppercase" value="{{ $size }}">{{ strtoupper($size) }}</option>
                                            @endforeach
