@props([
    ...
    'invert' => false,
    'dot_align' => config('bladewind.bell.dot_align', 'right'),
...
<div {{ $attributes->merge(['class' => 'bw-bell relative inline-block']) }}>
    {{-- <x-bladewind::icon name="bell"
        class="{{ $sizing[$size]['bell'] }} {{ $class }} cursor-pointer {{ $invert_css }}" /> --}}
    <x-bell />
    @if ($show_dot)
        <div
            class="{{ $sizing[$size]['dot'] }} rounded-full bg-{{ $colour }}-500 absolute top-0 @if ($dot_align == 'right') ml-4 @endif ltr:right-[2.5px] rtl:left-[2.5px] @if ($animate_dot) animate-ping @endif">
        </div>
    @endif
</div>
